...
fetch_merged_commits() {
  # Create a list of all public non-forked repos owned by the user
  fetch_all_public_repos > owned_repos.txt

  # Fetch all repositories with merged PRs
  curl -s -H "Authorization: token $GH_TOKEN" \
    ...
    while IFS="|" read -r repo url title; do
      ...
      echo "$repo|$language" >> repos.txt  # Store language for all repos, including third-party
